...
/**
 * This GUI will be act as the "startup" for all the other GUIs. Running this
 * View should set up all the GUI listeners and immediately load the first GUI.
 * 
 * The general idea of how GUI views works is that, rather than just interacting
 * with each other, they will also all be listeners to the main program. This
 * way, they are guaranteed to be synchronized.
 * 
 * @authors Henry, David
 *
 */
public class GUIController implements ModelEventListener {
	...
	public void recieveBoards(Iterator<Board> boards) throws StateException {
		if (this.currentState instanceof DashBoardGUI) {
			...
			ErrorGUI.showError("State Error",
					"Must be in DashBoard to recieve boards.");
		}
